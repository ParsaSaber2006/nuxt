<template>
  ...
  <v-divider class="my-2" />
        ...
        <v-list-item :to="{ name: 'Users' }" color="primary" class="py-3">
          <template #prepend>
  ...
  <v-divider inset></v-divider>
   ...
   <v-list-item :to="{ name: 'Calls' }" color="primary" class="py-3" v-if="isAdmin">
          <template #prepend>
...
try {
  const response = await axios.post('/api/user/contacts/xml', {}, {
    headers: {
